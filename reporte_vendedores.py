# -*- coding: utf-8 -*-
"""reporte_semanal_vendedores_cheques_v2.ipynb
...
2. Conexión a Odoo
3. Bloque A – Ventas últimos 7 días
4. Bloque B – Pedidos no entregados (stock)
5. Bloque C – Pedidos pendientes de pago
6. **Bloque D – Cobranza: account.move.line (FAC+APER) + zonas + PDF**
7. Bloque E – Lo que viene (15 días, padre/hijo)
8. **Bloque H – Email resumen ejecutivo (Natalia + Daniel)**
9. Bloque F – Email individual por vendedor + envío

> Al correr el Bloque D imprime las ciudades sin zona para que las puedas agregar.

**v2 cambios:** Cobranza corregida (cheques PCSH2 descontados), email María Elena (top 30 cobranza + PDF con vendedor).
...
# ── Odoo ──────────────────────────────────────
ODOO3_URL  = "https://temponovo.odoo.com"
ODOO3_DB   = "cmcorpcl-temponovo-main-24490235"
ODOO3_USER = "natalia@temponovo.cl"
ODOO3_PASS = "Contraodoo94+"
...
# ── Quiénes reciben el resumen completo ───────
NATALIA_EMAILS = [
    "natalia@temponovo.cl",
...
MARIA_ELENA_EMAIL = "mariaelena@temponovo.cl"

# ── Vendedores (name = exacto en Odoo) ────────
VENDEDORES = [
...
WIZ_MODEL   = "salesperson.accounting.wizard"
REPORT_NAME = "temponovo_salesperson_accounting_report.report_template"
LABEL_TIPO   = "Un Vendedor"
LABEL_ESTADO = "FA/ND No Pagadas"
CTX_ODOO    = {"lang": "es_CL", "tz": "America/Santiago"}

print("✅ Configuración cargada")
print(f"   SMTP     : {SMTP_HOST}:{SMTP_PORT} ({SMTP_USER})")
print(f"   Modo     : {'PRUEBA → ' + TEST_TO if TEST_MODE else 'REAL'}")

"""---
## Bloque 1b – Helpers y CSS (funciones reutilizables)
"""
...
"""---
## Bloque A – Ventas últimos 7 días
Trae `sale.order.line` de pedidos confirmados. Genera:
- `df_ventas` → todas las líneas (una fila por producto)
- Columnas: Vendedor, Fecha, Número, Cliente, Referencia, Pedido, Entregado, Descuento%, Precio, Total
"""
...
"""---
## Bloque B – Pedidos no entregados (diferencias de stock)
Detecta pedidos donde algunas líneas tienen `Entregado > 0` pero otras `Entregado = 0`.
Genera `df_diferencias` con los ítems pendientes.
"""
...
"""---
## Bloque C – Pedidos pendientes de pago
Pedidos en estado **Borrador** o **Enviado** con fecha anterior a hoy.
Incluye resumen de categorías de producto.
Genera `df_pendientes`.
"""
...
"""---
## Bloque D – PDF de cobranza (FA/ND No Pagadas)
Descarga en memoria el informe de cobranza por vendedor desde el módulo personalizado de Odoo.
Genera `pdfs_cobranza` → dict `{nombre_vendedor: bytes}`
"""
...
if receivable_ids:
    ...
    raise RuntimeError("No se encontraron cuentas por cobrar.")

# ── 1b) Diario cheques por cobrar (PCSH2) ─────────────────────
# Los PCSH2 son cheques que el cliente ya entregó.
# Comercialmente ese monto ya está "pagado" → se descuenta de la deuda.
cheque_journal_ids = set()
try:
    jids = models.execute_kw(
        ODOO3_DB, uid, ODOO3_PASS,
        "account.journal", "search",
        [[("name", "ilike", "Cheques por cobrar")]],
        {"limit": 10}
    )
    cheque_journal_ids = set(jids)
    print(f"  Diario cheques por cobrar: IDs {jids}")
except Exception as e:
    print(f"  ⚠️  No se pudo cargar diario cheques: {e}")

# ── 1c) Líneas de deuda (excluye PCSH2 desde el search) ───────
aml_domain = [
    ("account_id",         "in",  receivable_ids),
    ("move_id.state",      "=",   "posted"),
    ("partner_id",         "!=",  False),
    ("full_reconcile_id",  "=",   False),
    ("move_id.journal_id", "not in", list(cheque_journal_ids)),
]
...
if aml_ids:
    ...
    # ✅ Solo amount_residual > 0
    aml_list = [
        l for l in aml_list
        if not l.get("full_reconcile_id")
        and float(l.get("amount_residual") or 0) > 0
    ]
    print(f"Líneas por cobrar con saldo: {len(aml_list)}")

# ── 1d) Créditos de cheques PCSH2 por partner ─────────────────
pcsh_domain = [
    ("account_id",         "in",  receivable_ids),
    ("move_id.state",      "=",   "posted"),
    ("partner_id",         "!=",  False),
    ("full_reconcile_id",  "=",   False),
    ("move_id.journal_id", "in",  list(cheque_journal_ids)),
]
pcsh_aml_ids = models.execute_kw(
    ODOO3_DB, uid, ODOO3_PASS,
    "account.move.line", "search",
    [pcsh_domain], {"limit": 200000}
)
print(f"Líneas PCSH2 (cheques en cartera): {len(pcsh_aml_ids)}")

cheque_credito_pid = {}  # partner_id → crédito total a descontar
if pcsh_aml_ids:
    pcsh_aml_list = models.execute_kw(
        ODOO3_DB, uid, ODOO3_PASS,
        "account.move.line", "read",
        [pcsh_aml_ids],
        {"fields": ["partner_id", "amount_residual"]}
    )
    for l in pcsh_aml_list:
        pid     = m2o_id(l.get("partner_id"))
        saldo   = float(l.get("amount_residual") or 0)
        credito = abs(saldo) if saldo < 0 else 0
        if pid and credito > 0:
            cheque_credito_pid[pid] = cheque_credito_pid.get(pid, 0) + credito
print(f"  Partners con crédito cheques: {len(cheque_credito_pid)}")

# ── 2) Prefetch asientos ──────────────────────────────────────
move_ids_d = list({m2o_id(l.get("move_id")) for l in aml_list
                   if m2o_id(l.get("move_id"))})
move_map = {}
if move_ids_d:
...
# ── 3) Prefetch ciudad y nombre de partners ───────────────────
partner_ids_d = list({m2o_id(l.get("partner_id")) for l in aml_list
                      if m2o_id(l.get("partner_id"))})
city_map         = {}
partner_name_map = {}  # nombre_upper → partner_id
if partner_ids_d:
    partners_d = models.execute_kw(
        ODOO3_DB, uid, ODOO3_PASS,
        "res.partner", "read",
        [partner_ids_d], {"fields": ["id", "name", "city"]}
    )
    for p in partners_d:
        city_map[p["id"]]                           = (p.get("city") or "").strip()
        partner_name_map[p["name"].strip().upper()] = p["id"]

# ── 4) Zonas ──────────────────────────────────────────────────
CIUDADES = {
    "arica": (0,0), "parinacota": (0,1), "iquique": (0,2),
    "alto hospicio": (0,3), "pozo almonte": (0,4), "tocopilla": (0,5),
    "calama": (0,6), "antofagasta": (0,7), "mejillones": (0,8),
    "taltal": (0,9), "chañaral": (0,10), "chanarral": (0,10),
    "copiapó": (0,11), "copiapo": (0,11), "vallenar": (0,12),
    "huasco": (0,13), "freirina": (0,14), "la serena": (0,15),
    "coquimbo": (0,16), "ovalle": (0,17), "combarbala": (0,18),
    "illapel": (0,19), "salamanca": (0,20), "los vilos": (0,21),
    "valparaíso": (1,0), "valparaiso": (1,0), "viña del mar": (1,1),
    "vina del mar": (1,1), "quillota": (1,2), "la calera": (1,3),
    "hijuelas": (1,4), "llaillay": (1,5), "san felipe": (1,6),
    "los andes": (1,7), "cabildo": (1,8), "la ligua": (1,9),
    "casablanca": (1,10), "san antonio": (1,11), "villa alemana": (1,12),
    "quilpué": (1,13), "quilpue": (1,13), "limache": (1,14),
    "santiago": (1,15), "providencia": (1,16), "las condes": (1,17),
    "vitacura": (1,18), "ñuñoa": (1,19), "nunoa": (1,19),
    "maipú": (1,20), "maipu": (1,20), "pudahuel": (1,21),
    "quilicura": (1,22), "recoleta": (1,23), "independencia": (1,24),
    "la florida": (1,25), "puente alto": (1,26), "san bernardo": (1,27),
    "buin": (1,28), "paine": (1,29), "melipilla": (1,30),
    "talagante": (1,31), "peñaflor": (1,32), "penaflor": (1,32),
    "colina": (1,33), "lampa": (1,34), "rancagua": (1,35),
    "san vicente tt": (1,36), "san vicente": (1,36), "peumo": (1,37),
    "san fernando": (1,38), "santa cruz": (1,39), "rengo": (1,40),
    "hualañe": (1,41), "hualane": (1,41), "curicó": (1,42),
    "curico": (1,42), "molina": (1,43), "talca": (1,44),
    "maule": (1,45), "san javier": (1,46), "linares": (1,47),
    "parral": (1,48), "cauquenes": (1,49), "constitución": (1,50),
    "constitucion": (1,50), "chillán": (1,51), "chillan": (1,51),
    "chillán viejo": (1,52), "chillan viejo": (1,52),
    "san carlos": (1,53), "cabrero": (1,54),
    "los angeles": (2,0), "laja": (2,1), "angol": (2,2),
    "traiguen": (2,3), "traiguén": (2,3), "curacautin": (2,4),
    "curacautín": (2,4), "concepción": (2,5), "concepcion": (2,5),
    "talcahuano": (2,6), "tome": (2,7), "tomé": (2,7),
    "hualpén": (2,8), "hualpen": (2,8), "coronel": (2,9),
    "lota": (2,10), "lebu": (2,11), "cañete": (2,12), "canete": (2,12),
    "arauco": (2,13), "curanilahue": (2,14), "temuco": (2,15),
    "lautaro": (2,16), "victoria": (2,17), "nueva imperial": (2,18),
    "cholchol": (2,19), "cholchól": (2,19), "tolten": (2,20),
    "toltén": (2,20), "villarrica": (2,21), "villarica": (2,21),
    "pucón": (2,22), "pucon": (2,22), "loncoche": (2,23),
    "pitrufquén": (2,24), "pitrufquen": (2,24), "valdivia": (2,25),
    "la unión": (2,26), "la union": (2,26), "rio bueno": (2,27),
    "osorno": (2,28), "puerto octay": (2,29), "frutillar": (2,30),
    "puerto montt": (2,31), "puerto varas": (2,32), "castro": (2,33),
    "ancud": (2,34), "quemchi": (2,35), "quellón": (2,36),
    "quellon": (2,36), "futaleufu": (2,37), "futaleufú": (2,37),
    "futalefú": (2,37), "coyhaique": (2,38), "coihaique": (2,38),
...
df_cobr_raw = (
    df_cobr_raw.groupby(
        ["Factura","Doc_label","Move_name","Cliente","Partner_id",
         "Ciudad","Zona_idx","Zona_ord","Vendedor",
         "Fecha_factura","Fecha_venc","Dias_vencido","Bucket"],
        as_index=False
    ).agg(Saldo=("Saldo","sum"))
)
...
# ── 8) Pivot + descuento cheques → df_cobranza ───────────────
...
for col in ["A_la_fecha", "d_1_30", "Antiguos"]:
...
def restar_credito(row, credito):
    row = row.copy()
    for col in [">30", "1-30", "A la fecha"]:
        if credito <= 0:
            break
        if row[col] > 0:
            descuento  = min(row[col], credito)
            row[col]  -= descuento
            credito   -= descuento
    return row
...
for _, row in pivot.iterrows():
    credito = cheque_credito_pid.get(row.get("Partner_id"), 0)
    if credito > 0:
        row = restar_credito(row, credito)
    rows_ajustadas.append(row)

pivot = pd.DataFrame(rows_ajustadas)
...
# ── COL_W con columna Vendedor extra ──────────────────────────
COL_W_VEND   = [7.5*cm, 3.5*cm, 2.8*cm, 2.8*cm, 2.8*cm, 3*cm]   # con vendedor
COL_W_NOVEND = [10*cm,          2.8*cm, 2.8*cm, 2.8*cm, 3*cm]    # sin vendedor
HEADERS_VEND   = ["Cliente / Documento", "Vendedor", "A la fecha", "1-30", ">30 días", "Total"]
HEADERS_NOVEND = ["Cliente / Documento",             "A la fecha", "1-30", ">30 días", "Total"]
...
def make_zona_sep(nombre, color, col_w):
    n = len(col_w)
    data = [[Paragraph(nombre, st_zona)] + [""]*(n-1)]
...
def build_pdf_cobranza(df_vend_res, df_raw_vend, nombre_vendedor, mostrar_vendedor=False):
    """
    mostrar_vendedor=True  → PDF general con columna Vendedor (para María Elena)
    mostrar_vendedor=False → PDF individual por vendedor (sin columna Vendedor)
    """
    col_w   = COL_W_VEND   if mostrar_vendedor else COL_W_NOVEND
    headers = HEADERS_VEND if mostrar_vendedor else HEADERS_NOVEND
    ...
    # Solo clientes con total neto > 0
    clientes_validos = set(df_vend_res["Cliente"].str.strip())

    base = (df_raw_vend[
                df_raw_vend["Cliente"].str.strip().isin(clientes_validos)
            ][["Cliente","Ciudad","Zona_idx","Zona_ord","Vendedor"]]
            .drop_duplicates("Cliente").copy()
            .sort_values(["Zona_idx","Zona_ord","Cliente"]))
    ...
    for _, cli_row in base.iterrows():
        ...
        vendedor_cli = cli_row["Vendedor"]

        if zona_idx != zona_actual:
            if zona_actual is not None:
                story.append(Spacer(1, 0.4*cm))
            story.append(make_zona_sep(
                ZONA_NOMBRES.get(zona_idx, "📍  SIN ZONA"),
                ZONA_COLORS.get(zona_idx, colors.HexColor("#555555")),
                col_w
            ))
        ...
        tiene_mayor30 = (
            not row_res.empty
            and float(row_res[">30"].values[0]) > 0
        )

        ciudad_tag = f" <font size='7' color='#888888'>{ciudad}</font>" if ciudad else ""
        ...
        if not row_res.empty:
            ...
            if mostrar_vendedor:
                fila_cli = [cli_label,
                            Paragraph(vendedor_cli or "", st_cell),
            ...
            fila_cli = [cli_label] + ([""] * (5 if mostrar_vendedor else 4))
        ...
        for _, inv in df_cli.iterrows():
            ...
            fila_base = [
                ...
                fmt(s) if b == "Antiguos"   else "",
                fmt(s),
            ]
            if mostrar_vendedor:
                fila_base.insert(1, "")   # celda vacía en col Vendedor para filas de doc
            filas_fac.append(fila_base)

        data_grp = [headers, fila_cli] + filas_fac
        tbl = Table(data_grp, colWidths=col_w, repeatRows=1)

        # col índices según modo
        col_af  = 2 if mostrar_vendedor else 1
        col_v30 = 4 if mostrar_vendedor else 3
        ...
        cmds = [
            ...
            ("ALIGN",         (col_af,0),(-1,-1), "RIGHT"),
            ("ALIGN",         (0,0),(col_af-1,-1),  "LEFT"),
            ("FONTSIZE",      (0,0),(-1,-1), 8),
        ...
        if tiene_mayor30:
            cmds += [("BACKGROUND", (0,1),(-1,1),     VENC_BG),
                     ("TEXTCOLOR",  (col_v30,1),(-1,1), VENC_TXT)]
        for fi, (_, inv) in enumerate(df_cli.iterrows(), start=2):
            if inv["Bucket"] == "Antiguos":
                cmds += [("BACKGROUND", (0,fi),(-1,fi),       VENC_BG),
                         ("TEXTCOLOR",  (col_af,fi),(-1,fi),  VENC_TXT)]
...
# ── 10) Generar PDFs individuales (sin columna vendedor) ──────
pdfs_cobranza = {}
for v in VENDEDORES:
    nombre   = v["name"]
    ...
    if df_v_res.empty:
        continue
    try:
        pdfs_cobranza[nombre] = build_pdf_cobranza(
            df_v_res, df_v_raw, nombre, mostrar_vendedor=False
        )
        print(f"  ✅ PDF {nombre} → {len(pdfs_cobranza[nombre]):,} bytes")
    except Exception as e:
        print(f"  ⚠️  {nombre}: {e}")

# ── PDF general con vendedor (para María Elena y resumen) ─────
try:
    pdf_general = build_pdf_cobranza(
        df_cobranza, df_cobr_raw, "Cobranza General", mostrar_vendedor=True
    )
    print(f"  ✅ PDF general (con vendedor) → {len(pdf_general):,} bytes")
except Exception as e:
    pdf_general = None
...
"""---
## Bloque E – Lo que viene (desde Pedidos de COMPRA)

Trae pedidos de compra confirmados (`purchase.order`) con **fecha de entrega esperada** en los **próximos 30 días**.
Agrupa por **fecha de entrega + categoría padre** para generar frases del tipo:
> *"Llegan el 10-02-2026: Calculadoras Casio (100 unidades), Relojes Casio (50 unidades)"*

Genera `df_lo_que_viene` y `html_lo_que_viene`.
"""
...
"""---
## Bloque H – Email resumen ejecutivo (Natalia + Daniel)
...
def build_email_resumen(df_ventas, df_cobranza, df_cobr_raw,
    ...
    if not df_cobranza.empty and ">30" in df_cobranza.columns:
        ...
        top10 = (
            df_cobranza[df_cobranza[">30"] > 0].copy()
            .assign(
                dias_max=df_cobranza[df_cobranza[">30"] > 0]["Cliente"].map(
                    df_cobr_raw[df_cobr_raw["Bucket"].isin(["Antiguos"])]
    ...
    if not df_ventas.empty:
        ...
        df_ped["Fecha_pedido_str"] = df_ped["Fecha_pedido"].dt.strftime("%d-%m-%Y") \
            if hasattr(df_ped["Fecha_pedido"], "dt") else df_ped["Fecha_pedido"]
        df_pedidos_resumen = (
...
# ── Email María Elena: solo cobranza top 30 ───────────────────
def build_email_maria_elena(df_cobranza, df_cobr_raw):
    """
    - Solo cobranza, sin ventas ni lo que viene
    - Top 30 clientes más graves
    - Tabla con columna Vendedor
    - Naranja suave para >30 días, rojo solo si >60 días
    """
    ...
    df_top30 = (
        df_cobranza.copy()
        .assign(
            dias_max=df_cobranza["Cliente"].map(
                df_cobr_raw[df_cobr_raw["Bucket"] == "Antiguos"]
                .groupby("Cliente")["Dias_vencido"].max()
            ).fillna(0)
        )
        .sort_values([">30", "dias_max", "Total"], ascending=[False, False, False])
        .head(30)
    )

    def fmt_me(v, dias_max=0):
        v = float(v) if v is not None else 0.0
        if v == 0:
            return "<td style='text-align:right;color:#ccc'>–</td>"
        if dias_max > 60:
            st = "color:#c0392b;font-weight:bold"   # rojo solo >60 días
        elif dias_max > 30:
            st = "color:#e65100;font-weight:bold"   # naranja 31-60 días
        else:
            st = ""
        return f"<td style='text-align:right;{st}'>$ {format_clp(v)}</td>"
    ...
    for _, row in df_top30.iterrows():
        dias      = int(row.get("dias_max", 0))
        tiene_v30 = float(row.get(">30", 0)) > 0

        # Color de fila: naranja suave 31-60, rojo suave solo >60
        if dias > 60:
            tr_style = "background:#fde8e8;"
        elif dias > 30:
            tr_style = "background:#fff3e0;"
        else:
            tr_style = ""
        ...
        badge = (
            ...
            f"border-radius:3px;padding:1px 5px;font-size:11px'>{dias}d</span> "
            if dias > 0 and badge_bg else ""
        )

        rows_html += (
            f"<tr style='{tr_style}'>"
            f"<td>{badge}<b>{row['Cliente']}</b>"
            f"<br><small style='color:#888'>{row['Ciudad']}</small></td>"
            f"<td style='font-size:12px'>{row['Vendedor']}</td>"
            + fmt_me(float(row.get("A la fecha", 0)))
            + fmt_me(float(row.get("1-30", 0)))
            + fmt_me(float(row.get(">30", 0)), dias_max=dias)
            + fmt_me(float(row.get("Total", 0)))
            + "</tr>"
        )

    subject = f"💰 Cobranza – Top 30 | {hoy_str}"
    html = f"""
    <html><head>{BASE_CSS}</head><body><div class="wrap">
      <h2>💰 Cobranza pendiente – Top 30 más graves</h2>
      <p>Al <b>{hoy_str}</b></p>
      <div class="kpi" style="border-color:#fb8c00">
        Total empresa: <b>$ {format_clp(total_all)}</b>
        &nbsp;·&nbsp; Vencido >30 días: <b style="color:#e65100">$ {format_clp(total_v30)}</b>
        &nbsp;·&nbsp; <i>PDF adjunto con detalle completo por vendedor</i>
      </div>
      <table>
        <thead><tr>
      ...
      <p style='font-size:11px;color:#888'>
        🟠 Naranja = 31-60 días vencido &nbsp;·&nbsp; 🔴 Rojo = más de 60 días vencido<br>
        El badge indica días de antigüedad máxima del vencimiento.<br>
        PDF adjunto contiene el detalle completo ordenado por vendedor.
      </p>
      <p class="footer">Reporte generado automáticamente desde Odoo.</p>
    </div></body></html>"""
...
print("✅ build_email_resumen y build_email_maria_elena listos")

# ── PDF María Elena: vendedor → zona → cliente ────────────────
def build_pdf_maria_elena(df_cobranza, df_cobr_raw):
    """
    PDF para María Elena:
    - Banda gris oscuro por vendedor (nivel superior)
    - Bandas de zona adentro de cada vendedor (igual al PDF normal)
    - Naranja para 31-60 días, rojo solo >60 días
    - Sin columna vendedor (ya está agrupado por sección)
    """
    VEND_HDR_BG  = colors.HexColor("#3d3d3d")
    VEND_HDR_TXT = colors.white
    VENC_BG_NARANJA = colors.HexColor("#fff3e0")
    VENC_BG_ROJO    = colors.HexColor("#fde8e8")
    VENC_TXT_NARANJA = colors.HexColor("#e65100")
    VENC_TXT_ROJO    = colors.HexColor("#c0392b")

    col_w   = [10*cm, 3*cm, 3*cm, 3*cm, 3.2*cm]
    headers = ["Cliente / Documento", "A la fecha", "1-30", ">30 días", "Total"]

    st_vend = ParagraphStyle("vend", parent=styles["Normal"], fontSize=11,
                              fontName="Helvetica-Bold", textColor=VEND_HDR_TXT)

    clientes_validos = set(df_cobranza["Cliente"].str.strip())
    ...
    doc = SimpleDocTemplate(buf, pagesize=landscape(A4),
                            ...
                            topMargin=1.2*cm,  bottomMargin=1.2*cm)
    story = []
    story.append(Paragraph("Cobranza pendiente – Todos los vendedores", st_h1))
    story.append(Paragraph(
        f"Al {hoy_dt.strftime('%d/%m/%Y')}  |  Ordenado por Vendedor → Zona → Cliente", st_sub))

    # Ordenar: vendedor → zona_idx → zona_ord → cliente
    base_todos = (
        df_cobr_raw[df_cobr_raw["Cliente"].str.strip().isin(clientes_validos)]
        [["Cliente","Vendedor","Ciudad","Zona_idx","Zona_ord"]]
        .drop_duplicates("Cliente")
        .sort_values(["Vendedor","Zona_idx","Zona_ord","Cliente"])
    )

    vendedor_actual = None
    zona_actual     = None

    for _, cli_row in base_todos.iterrows():
        cliente    = cli_row["Cliente"]
        vendedor   = cli_row["Vendedor"]
        zona_idx   = int(cli_row["Zona_idx"])
        ciudad     = cli_row["Ciudad"]
        ...
        if vendedor != vendedor_actual:
            ...
            # Banda gris oscuro con nombre vendedor
            data_vend = [[Paragraph(f"👤  {vendedor}", st_vend), "", "", "", ""]]
            tbl_vend  = Table(data_vend, colWidths=col_w)
            tbl_vend.setStyle(TableStyle([
                ("BACKGROUND",    (0,0),(-1,0), VEND_HDR_BG),
                ("SPAN",          (0,0),(-1,0)),
                ("TOPPADDING",    (0,0),(-1,0), 8),
                ("BOTTOMPADDING", (0,0),(-1,0), 8),
                ("LEFTPADDING",   (0,0),(-1,0), 12),
            ]))
            story.append(tbl_vend)
            story.append(Spacer(1, 0.1*cm))
            vendedor_actual = vendedor
            zona_actual     = None   # reset zona al cambiar vendedor

        # ── Separador de zona (dentro del vendedor) ───────────
        if zona_idx != zona_actual:
        ...
        # ── Tabla del cliente ──────────────────────────────────
        df_cli  = df_cobr_raw[df_cobr_raw["Cliente"]==cliente].sort_values("Fecha_factura")
        row_res = df_cobranza[df_cobranza["Cliente"]==cliente]

        dias_max_cli = int(
            df_cobr_raw[
                (df_cobr_raw["Cliente"]==cliente) &
                (df_cobr_raw["Bucket"]=="Antiguos")
            ]["Dias_vencido"].max()
        ) if not df_cobr_raw[
            (df_cobr_raw["Cliente"]==cliente) &
            (df_cobr_raw["Bucket"]=="Antiguos")
        ...
        if dias_max_cli > 60:
            cli_venc_bg  = VENC_BG_ROJO
            cli_venc_txt = VENC_TXT_ROJO
        elif dias_max_cli > 30:
            cli_venc_bg  = VENC_BG_NARANJA
            cli_venc_txt = VENC_TXT_NARANJA
        else:
            cli_venc_bg  = CLI_BG
            cli_venc_txt = colors.black
        ...
        cmds = [
            ...
            ("TEXTCOLOR",     (0,0),(-1,0),  HDR_TXT),
            ("FONTNAME",      (0,0),(-1,0),  "Helvetica-Bold"),
            ("BACKGROUND",    (0,1),(-1,1),  cli_venc_bg),
            ("FONTNAME",      (0,1),(-1,1),  "Helvetica-Bold"),
            ("ROWBACKGROUNDS",(0,2),(-1,-1), [colors.white, ALT_BG]),
            ("GRID",          (0,0),(-1,-1), 0.4, GRID_COLOR),
            ("ALIGN",         (1,0),(-1,-1), "RIGHT"),
        ...
        # Color texto >30 en fila cliente según antigüedad
        if dias_max_cli > 0:
            cmds += [("TEXTCOLOR", (3,1),(-1,1), cli_venc_txt)]

        # Filas de documentos: naranja o rojo según antigüedad
        for fi, (_, inv) in enumerate(df_cli.iterrows(), start=2):
            if inv["Bucket"] == "Antiguos":
                dias_inv = int(inv.get("Dias_vencido", 0))
                bg  = VENC_BG_ROJO    if dias_inv > 60 else VENC_BG_NARANJA
                txt = VENC_TXT_ROJO   if dias_inv > 60 else VENC_TXT_NARANJA
                cmds += [("BACKGROUND", (0,fi),(-1,fi), bg),
                         ("TEXTCOLOR",  (1,fi),(-1,fi), txt)]
...
print("✅ build_pdf_maria_elena lista")

"""---
## Bloque F – Armado del email y envío
Construye el email completo por vendedor y lo envía con:
- Adjunto Excel con detalle de ventas
- Adjunto PDF de cobranza
"""
...
# ── Resumen ejecutivo (Natalia + Daniel) ──────────────────────
try:
    ...
    if not info_res["df_pedidos"].empty:
        ...
        os.close(fd2)
        try:
            export_excel_autowidth(info_res["df_pedidos"], tmp_res, "Pedidos semana")
...
# ── Email María Elena: solo cobranza top 30 ───────────────────
try:
    info_me = build_email_maria_elena(df_cobranza, df_cobr_raw)

    if TEST_MODE:
        to_me  = TEST_TO[0] if isinstance(TEST_TO, list) else TEST_TO
        cc_me  = TEST_TO[1:] if isinstance(TEST_TO, list) else []
    else:
        to_me = MARIA_ELENA_EMAIL
        cc_me = []

    # Adjunto: PDF general con columna vendedor
    attachments_me = []
    # PDF especial para María Elena: ordenado por vendedor, colores suaves
    try:
        pdf_me = build_pdf_maria_elena(df_cobranza, df_cobr_raw)
        attachments_me.append({
            "data":     pdf_me,
            "filename": f"cobranza_completa_{pd.Timestamp.now().strftime('%Y-%m-%d')}.pdf"
        })
    except Exception as e_pdf:
        print(f"  ⚠️  PDF María Elena falló, usando general: {e_pdf}")
        if pdf_general:
            attachments_me.append({
                "data":     pdf_general,
                "filename": f"cobranza_completa_{pd.Timestamp.now().strftime('%Y-%m-%d')}.pdf"
    ...
    send_email(to_me, cc_me, info_me["subject"], info_me["html"], attachments_me)
    modo = "PRUEBA" if TEST_MODE else "REAL"
    print(f"[{modo}] María Elena → {to_me}  (adjunto: {len(attachments_me)} archivos)")
except Exception as e:
